...
export const RecordsView: React.FC<RecordsViewProps> = ({
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, selectedBarangay, selectedPurpose, selectedHealth, syncFilter, pageSize]);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
            ...
            <div>
              ...
              >
                ...
                <option value="deceased">Deceased / Mortality Events Only</option>
              </select>
            </div>
          </div>
        </div>
              ...
              {paginatedPigs.map(pig => {
                ...
                return (
                  <div key={`card-${pig.id}`} className="p-4 bg-white hover:bg-[#FBF8EF] transition-colors space-y-3">
                    {/* Header: Tag + Purpose + Health Badge */}
                    ...
                    {/* Biosecurity Score Banner */}
                    <div className="flex items-center justify-between bg-[#F5EFDD]/70 border border-[#DED2AE] px-2.5 py-1.5 rounded-xl text-xs">
                      <span className="text-[#203F2B] font-semibold flex items-center gap-1.5 text-[11px]">
                        <ShieldCheck className="w-3.5 h-3.5 text-[#2F5C3F]" />
                        Biosecurity Assessment:
                      </span>
                      <span className={`font-mono font-bold text-[11px] px-2 py-0.5 rounded ${
                        bioScore >= 6 ? 'bg-emerald-100 text-emerald-800' : bioScore >= 4 ? 'bg-[#EAE1C4] text-[#2F5C3F]' : 'bg-amber-100 text-amber-900'
                      }`}>
                        {bioScore}/7 Biosecure
                      </span>
                    </div>
